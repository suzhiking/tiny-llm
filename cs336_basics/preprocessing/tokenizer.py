...
def pretokenize(input_path: str, special_tokens: list[str]) -> dict[str, int]:
    ...
    with open(input_path, "rb") as f:
        ...
        num_processes = 4 if file_size_mb > 200 else 1
...
def train_bpe(
    ...
    if output_path:
        ...
        vocab_output_path = output_path + "/train-bpe-vocab.json"
        merges_output_path = output_path + "/train-bpe-merges.json"
        ...
        with open(merges_output_path, "w") as json_file:
            json.dump(serializable_merges, json_file, indent=4)
...
class Tokenizer:
    ...
    def _merge(self, token: tuple[bytes]):
