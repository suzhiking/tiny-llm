...
class RoPE(nn.Module):
    def __init__(self, theta: float, d_k: int, max_seq_len: int, device=None):
        ...
        self.register_buffer("sin_val", sin_val.to(device=device))
        self.register_buffer("cos_val", cos_val.to(device=device))
    ...
    def forward(
        ...
        # If sin/cos are missing leading dims, add them as size-1 so broadcasting works
        # while sin.dim() < x_even.dim():
        #     sin = sin.unsqueeze(0)
        #     cos = cos.unsqueeze(0)
    ...
    def multi_head(
        ...
        if self.pos_encode:
            if self.token_positions is None:
                token_positions = torch.arange(seq_len).view(new_shape).expand(*prev_shape, seq_len)
            else:
